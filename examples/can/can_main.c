 ...
 * Private Functions
 ****************************************************************************/
static void show_usage(FAR char const* progname) {
    fprintf(stderr,
            "USAGE: %s"
            " [-n <nmsgs]"
            #ifdef CONFIG_EXAMPLES_CAN_WRITE
    ...
    for (msgno = 0; !nmsgs || msgno < nmsgs; msgno++) {
        ...
        #endif
        {
            ...
            if (memcmp(&txmsg.cm_hdr, &rxmsg.cm_hdr, sizeof(struct can_hdr_s)) != 0) {
                ...
                lib_dumpbuffer("Sent header", (FAR uint8_t const*)&txmsg.cm_hdr, sizeof(struct can_hdr_s));
                lib_dumpbuffer("Received header", (FAR uint8_t const*)&rxmsg.cm_hdr, sizeof(struct can_hdr_s));
                errval = 4;
                goto errout_with_dev;
            }
